 ...
 * Second line: sub-agent propagation — UserPromptSubmit never fires for
 * Agent/AgentSwarm spawns, so the main agent must copy the protocol into
 * each sub-agent prompt itself (soft rule; PreToolUse can only allow/deny,
 * not rewrite tool input).
 */
export const PROTOCOL_TAIL =
  'Reasoning Protocol: think strictly in English and begin with "We need...".\n' +
  'Sub-agents: when you spawn sub-agents (Agent / AgentSwarm) that will run on a DeepSeek V4 model, copy the Reasoning Protocol above verbatim to the top of each sub-agent prompt.'
