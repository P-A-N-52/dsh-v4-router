...
3. 第一条消息注入 Reasoning Protocol（家族中性，永远安全）；persona 配方（w7/w6c）等 wire 确认真实模型家族后注入（通常第二条消息起）；之后每条消息注入一条固定引导（按任务复杂度二选一，缓存友好）；
...
- 门控以会话 wire 记录里的**真实服务模型**为准（SessionStart 事件只上报配置默认模型，看不到 `-m` 或会话内选择）。第一条消息时 wire 尚未写盘，此时只注入家族中性的 Reasoning Protocol——**persona 刻意迟到一条**，因为给错家族是实测里最伤的情况且路径提交会让它粘住；第二条约到不损效果（P14/P19 近场零衰减）。非 v4 会话从第二条消息起自动静默。想完全不受打扰用 `/dsh-v4-router:off`。
