...
- 门控以会话 wire 记录里的**真实服务模型**为准（SessionStart 事件只上报配置默认模型，看不到 `-m` 或会话内选择）。每会话**第一条消息**时 wire 尚未写盘、无证据可言，此时一律注入（fail-open 朝向插件目的）；若首条按错误家族给了协议块（默认模型与实际模型不同族），第二条消息会**自动注入一次纠正块**换成正确配方；非 v4 会话从第二条消息起自动静默。想完全不受打扰用 `/dsh-v4-router:off`。
